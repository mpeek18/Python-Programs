...
def analyzeFits(finalName, fitsName, zGal, sciHeight, hAlphaFlux):
    ...
    fits.writeto('CROP-SDSS-J095432.63+354027.7-G141_00' + str(galID) + '.2D.fits', newImage, f[0].header, overwrite=True)
    f.close()
    ...
    plt.savefig('CROP-J095432.63+354027.7-G141_00' + str(galID) + '.png', dpi=100)
    plt.show()
...
"""
###############################################################################
csvFile = open('Best_redux_Target4.csv')
with open('Best_redux_Target4.csv') as csvFile:
    ...
    for row in readCSV:
        ...
        if (counter > 0): 
            ...
            if (row[6] == '0'):
                orientName.append('-07-304-') #Orientation 0
                
            elif (row[6] == '1'):
                orientName.append('-16-011-') #Orientation 1
...
with open('Abs_data_Target4.csv') as csvFile:
    readCSV = csv.reader(csvFile, delimiter=',')
    galQsoID = []
    galZQso = []
    mgII = []
...
for i in range(0, len(galaxyID)): #Loop through Galaxy ID #'s.    
    ...
    if (zQual[i] == 'likely' or zQual[i] == 'good' or zQual[i] == 'probable' 
        ...
        #zfits.dat file
        zFits = ascii.read('SDSS-J095432.63+354027.7' + str(frame) + 'G141_' + str(galID).zfill(5) + '.zfit.dat')     
        zGal = zFits['z_max_spec']
        print (zGal)
        
        #Hubble fits image
        fitsName = 'SDSS-J095432.63+354027.7' + str(frame) + 'G141_' + str(galID).zfill(5) + '.2D.fits'
        ...
        try:
            fitsModel = 'SDSS-J095432.63+354027.7' + str(frame) + 'G141_' + str(galID).zfill(5) + '.zfit.fits'
...
newGalID = ['288', '373', '295', '387', '208', '338', '329', '416', '306']
...
newAbsorberGalID = ['288', '387', '338', '329']
...
ascii.write(zDistAbsorbData, 'Field5_zDistAbsorb.dat', format='fixed_width', overwrite=True)

zDistNoAbsorbData = (Table([zDistNonAbsorb], names=['Redshift']))
ascii.write(zDistNoAbsorbData, 'Field5_zDistNoAbsorb.dat', format='fixed_width', overwrite=True)

r90AbsorbData = (Table([r90Absorb], names=['Radius 90']))
ascii.write(r90AbsorbData, 'Field5_r90Absorb.dat', format='fixed_width', overwrite=True)

#Table for combining r90 non-absroption data.
r90NoAbsorbData = (Table([r90NoAbsorb], names=['Radius 90']))
ascii.write(r90NoAbsorbData, 'Field5_r90NoAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining r50 absorption data.
r50AbsorbData = (Table([r50Absorb,], names=['Radius 50']))
ascii.write(r50AbsorbData, 'Field5_r50Absorb.dat', format='fixed_width', overwrite=True)

#Table for combining r50 non-absroption data.
r50NoAbsorbData = (Table([r50NoAbsorb], names=['Radius 50']))
ascii.write(r50NoAbsorbData, 'Field5_r50NoAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining Galaxy areas for absorbers
galAreaAbsorbData = (Table([galAreaAbsorb], names=['Galaxy Area']))
ascii.write(galAreaAbsorbData, 'Field5_galAreaAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining Galaxy areas for non-absorbers
galAreaNoAbsorbData = (Table([galAreaNoAbsorb], names=['Galaxy Area']))
ascii.write(galAreaNoAbsorbData, 'Field5_galAreaNoAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining sfrDensity data.
sfrAbsorbData = (Table([sfrDensAbsorb], names=['SFR Density Absorber']))
ascii.write(sfrAbsorbData, 'Field5_SFRDensAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining sfrDensity data.
sfrNoAbsorbData = (Table([sfrDensNoAbsorb], names=['SFR Density No Absorber']))
ascii.write(sfrNoAbsorbData, 'Field5_NoSFRDensAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining sfr data.
sfrAbsorbData = (Table([SFRAbsorb], names=['SFR Absorber']))
ascii.write(sfrAbsorbData, 'Field5_SFRAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining sfr data.
sfrNoAbsorbData = (Table([SFRNoAbsorb], names=['SFR Non Absorber']))
ascii.write(sfrNoAbsorbData, 'Field5_SFRNoAbsorb.dat', format='fixed_width', overwrite=True)
...
ascii.write(angDistData, 'Field5_AngularDistance.dat', format='fixed_width', overwrite=True)
...
ascii.write(angDistAbsorbData, 'Field5_AbsorbAngularDistance.dat', format='fixed_width', overwrite=True)
...
ascii.write(angDistNoAbsorbData, 'Field5_NoAbsorbAngularDistance.dat', format='fixed_width', overwrite=True)
...
ascii.write(data, 'HSTData5.dat', format='fixed_width', overwrite=True)

...
def analyzeFits(finalName, fitsName, zGal, sciHeight, hAlphaFlux):
    ...
    print ()
    return (radius50, radius90, fluxArray)
        ...
        if (galRedshift[i] < 1.6 and galRedshift[i] > 0.65):
            if (fluxHa[i] > 0 and fluxErrHa[i] > 0):
                if ((fluxHa[i] / fluxErrHa[i]) > 1): 
                    ...
                    if (frame == '-08-064-'):
                        positionAngle.append(64)
                    elif (frame == '-17-084-'):
                        positionAngle.append(84)
                    else:
                        positionAngle.append(84)
                    ...
                    try:
                        fitsModel = 'SDSS-J001453.19+091217.6' + str(frame) + 'G141_' + str(galID).zfill(5) + '.zfit.fits'
                    ...
                    radius50, radius90, fluxArray = analyzeFits(finalName, fitsName, zGal, sciHeight, hAlphaFlux)
...
absorberGalID = ['359', '351', '303', '392', '236', '344 ', '284', '427', '237', 
            '306', '216', '262', '274', '285', '317', '274', '221', '226', 
            '299', '364', '220', '419', '236', '317', '274', '221', '226']
#print ("absorberGalID array length:", len(absorberGalID))
...
newAbsorberGalID = ['359', '306', '285', '317', '274', '221']
print ("newAbsorberGalID length:", len(newAbsorberGalID))
print ("ID array length:", len(ID))
...
for i in range(0, len(SFRAbsorb)):
    if (SFRAbsorb[i] > 20 and SFRAbsorb[i] < 30):
        print ("Galaxy ID SFR = 25", galIDAbsorb[i])
...
ascii.write(zDistAbsorbData, 'Field1_zDistAbsorb.dat', format='fixed_width', overwrite=True)

zDistNoAbsorbData = (Table([zDistNonAbsorb], names=['Redshift']))
ascii.write(zDistNoAbsorbData, 'Field1_zDistNoAbsorb.dat', format='fixed_width', overwrite=True)

r90AbsorbData = (Table([r90Absorb], names=['Radius 90']))
ascii.write(r90AbsorbData, 'Field1_r90Absorb.dat', format='fixed_width', overwrite=True)

#Table for combining r90 non-absroption data.
r90NoAbsorbData = (Table([r90NoAbsorb], names=['Radius 90']))
ascii.write(r90NoAbsorbData, 'Field1_r90NoAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining r50 absorption data.
r50AbsorbData = (Table([r50Absorb,], names=['Radius 50']))
ascii.write(r50AbsorbData, 'Field1_r50Absorb.dat', format='fixed_width', overwrite=True)

#Table for combining r50 non-absroption data.
r50NoAbsorbData = (Table([r50NoAbsorb], names=['Radius 50']))
ascii.write(r50NoAbsorbData, 'Field1_r50NoAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining Galaxy areas for absorbers
galAreaAbsorbData = (Table([galAreaAbsorb], names=['Galaxy Area']))
ascii.write(galAreaAbsorbData, 'Field1_galAreaAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining Galaxy areas for non-absorbers
galAreaNoAbsorbData = (Table([galAreaNoAbsorb], names=['Galaxy Area']))
ascii.write(galAreaNoAbsorbData, 'Field1_galAreaNoAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining sfrDensity data.
sfrAbsorbData = (Table([sfrDensAbsorb], names=['SFR Density Absorber']))
ascii.write(sfrAbsorbData, 'Field1_SFRDensAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining sfrDensity data.
sfrNoAbsorbData = (Table([sfrDensNoAbsorb], names=['SFR Density Non Absorber']))
ascii.write(sfrNoAbsorbData, 'Field1_NoSFRDensAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining sfr data.
sfrAbsorbData = (Table([SFRAbsorb], names=['SFR Absorber']))
ascii.write(sfrAbsorbData, 'Field1_SFRAbsorb.dat', format='fixed_width', overwrite=True)
...
ascii.write(data, 'HSTData1.dat', format='fixed_width', overwrite=True)
...
ascii.write(absorberData, 'Absorption_Data_Field0.dat', format='fixed_width', overwrite=True)

...
def analyzeFits(finalName, fitsName, zGal, sciHeight, hAlphaFlux):
    ...
    #Same as above but without gaussian blur
    plt.clf()
...
def starFormation(hAlphaFlux, zGal):
    ...
    sfr = (7.9*10**-42)*lum
    print("Star Formation Rate:", sfr[0].value,"\n")
...
for i in range(0, len(galaxyID)): #Loop through Galaxy ID #'s.    
    ...
    sfr = starFormation(hAlphaFlux, zGal)
...
data = Table([ID, redshift, galRedshift, R50, R90, starForm], names=['Galaxy ID', 'zGals', 'csvRedshift', 'R50', 'R90', 'Star Formation'])

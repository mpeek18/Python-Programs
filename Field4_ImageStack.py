...
"""
def getGalAngle(ID):
    for i in range(0, len(field3IDs)):
        if (ID == field3IDs[i]):
            galAngle = field3Angles[i]
...
Find only galaxies for field 3 and append ID's and Angles to new list
...
for i in range(0, len(fields)):
    if (fields[i] == '3'):
        field3IDs.append(galaxyIDs[i])
        field3Angles.append(float(angles[i]))
        
print ("Field 3 ID's", field3IDs,'\n')
print ("Field 3 Angles", field3Angles,'\n')
...
ascii.write(stackDataAbsorbers, 'Field4_Stack_Data_Absorb.dat', format='fixed_width', overwrite=True)
...
ascii.write(stackDataNonAbsorbers, 'Field4_Stack_Data_NonAbsorb.dat', format='fixed_width', overwrite=True)
print ("Field4_Stack_Data file has been written")

...
"""
getGalAngle function takes galaxy ID as argument, goes through field1IDs list 
and matches passed argument. Finds associated galaxy angle and returns angle.
"""
def getGalAngle(ID):
    for i in range(0, len(field1IDs)):
        if (ID == field1IDs[i]):
            galAngle = field1Angles[i]
...
def stackAbsorber(fileListAbsorb):
    ...
    fits.writeto('Field2_Stacked_Image_Mean_Normed_Absorber.fits', meanImage, overwrite=True)
    fits.writeto('Field2_Stacked_Image_Median_Normed_Absorber.fits', medianImage, overwrite=True)
    fits.writeto('Field2_Stacked_Image_Normed_Absorber.fits', imageStack, overwrite=True)
...
def stackNonAbsorber(fileListNonAbsorb):
    ...
    fits.writeto('Field2_Stacked_Image_Mean_Normed_NonAbsorber.fits', meanImage, overwrite=True)
    fits.writeto('Field2_Stacked_Image_Median_Normed_NonAbsorber.fits', medianImage, overwrite=True)
    fits.writeto('Field2_Stacked_Image_Normed_NonAbsorber.fits', imageStack, overwrite=True)
...
def stackAll(fileListAll):
    ...
    fits.writeto('Field2_Stacked_Image_Mean_Normed_All.fits', meanImage, overwrite=True)
    fits.writeto('Field2_Stacked_Image_Median_Normed_All.fits', medianImage, overwrite=True)
    fits.writeto('Field2_Stacked_Image_Normed_All.fits', imageStack, overwrite=True)
...
Find only galaxies for field 1 and append ID's and Angles to new list
...
for i in range(0, len(fields)):
    if (fields[i] == '1'):
        field1IDs.append(galaxyIDs[i])
        field1Angles.append(float(angles[i]))
        
print ("Field 1 ID's", field1IDs,'\n')
print ("Field 1 Angles", field1Angles,'\n')
...
"""
absorberFile = ascii.read('Absorption_Data_Field1.dat', delimiter='|')
...
for i in range(1, len(ID)):
    if (ID[i] in field1IDs):
        if (absorber[i] == 'Yes'):
            ...
            try:
                fileName = 'CROP-SDSS-J082946.90+185222.0-G141_00' + ID[i] + '.2d.fits'
...
ascii.write(stackDataAbsorbers, 'Field2_Stack_Data_Absorb.dat', format='fixed_width', overwrite=True)

stackDataNonAbsorbers = (Table([objIDNonAbsorb, objRedshiftNonAbsorb], 
                               names=['ID Non-Absorber', 'Redshift Non-Absorber']))
ascii.write(stackDataNonAbsorbers, 'Field2_Stack_Data_NonAbsorb.dat', format='fixed_width', overwrite=True)
print ("Field2_Stack_Data file has been written")

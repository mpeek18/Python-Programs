...
for i in range(0, len(galaxyID)): #Loop through Galaxy ID #'s.    
    ...
    galID = galaxyID[i]
    zFits = ascii.read('goodsn-14-G141_' + str(galID) + '.new_zfit.dat')
    redshift = zFits['z_peak_grism']
    fitsName = 'goodsn-14-G141_' + str(galID) + '.2D.fits'
    finalName = 'Final-' + fitsName
    print ("Name:",fitsName)
    print (redshift)
    lineFitDat = 'goodsn-14-G141_' + str(galID) + '.linefit.dat'
    ...
    R50.append(radius50)
    R90.append(radius90)  
    starForming.append(sfr)
print () 
###############################################################################
#Write data to ascii table
data = Table([ID, redShifts, R50, R90, starForming], names=['Galaxy ID', 'Redshifts', 'R50', 'R90', 'SFR'])

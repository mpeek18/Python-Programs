...
def analyzeFits(finalName, fitsName, zGal, sciHeight, hAlphaFlux):
    ...
    fits.writeto('CROP-SDSS-J091730.18+324105.5-G141_00' + str(galID) + '.2D.fits', newImage, f[0].header, overwrite=True)
    f.close()
    ...
    plt.savefig('CROP-J091730.18+324105.5-G141_00' + str(galID) + '.png', dpi=100)
    plt.show()
...
###############################################################################
"""
csvFile = open('Best_redux_Target3.csv')
with open('Best_redux_Target3.csv') as csvFile:
    ...
    for row in readCSV:
        ...
        if (counter > 0): 
            ...
            if (row[6] == '0'):
                orientName.append('-01-312-') #Orientation 0
                
            elif (row[6] == '1'):
                orientName.append('-10-078-') #Orientation 1
...
with open('Abs_data_Target3.csv') as csvFile:
    readCSV = csv.reader(csvFile, delimiter=',')
    galQsoID = []
    galZQso = []
    mgII = []
...
"""
#################################################################################
with open('field3_matching.csv') as csvFile:
    readCSV = csv.reader(csvFile, delimiter=',')
    galaxyID = []
    orient = []
    orientName = []
...
for i in range(0, len(galaxyID)): #Loop through Galaxy ID #'s.    
    ...
    if (zQual[i] == 'likely' or zQual[i] == 'good' or zQual[i] == 'probable' 
        ...
        #zfits.dat file
        zFits = ascii.read('SDSS-J091730.18+324105.5' + str(frame) + 'G141_' + str(galID).zfill(5) + '.zfit.dat')     
        zGal = zFits['z_max_spec']
        print (zGal)
        
        #Hubble fits image
        fitsName = 'SDSS-J091730.18+324105.5' + str(frame) + 'G141_' + str(galID).zfill(5) + '.2D.fits'
        ...
        try:
            fitsModel = 'SDSS-J091730.18+324105.5' + str(frame) + 'G141_' + str(galID).zfill(5) + '.zfit.fits'
        ...
        #linefit.dat file
        try:
            lineFitDat = 'SDSS-J091730.18+324105.5' + str(frame) + 'G141_' + str(galID).zfill(5) + '.linefit.dat'    
            print (lineFitDat)
...
newGalID = ['427', '413', '452', '380', '445', '417', '343', '306']
...
plt.hist(sfrDensAbsorb, bins=np.linspace(0, 0.4, 10), density=True, histtype='step', label= 'MgII Detection (%i)' %len(sfrDensAbsorb))
plt.hist(sfrDensNoAbsorb, bins=np.linspace(0, 0.4, 10), density=True, histtype='step', label = 'MgII Non-Detection (%i)' %len(sfrDensNoAbsorb))
...
ascii.write(zDistAbsorbData, 'Field4_zDistAbsorb.dat', format='fixed_width', overwrite=True)

zDistNoAbsorbData = (Table([zDistNonAbsorb], names=['Redshift']))
ascii.write(zDistNoAbsorbData, 'Field4_zDistNoAbsorb.dat', format='fixed_width', overwrite=True)

r90AbsorbData = (Table([r90Absorb], names=['Radius 90']))
ascii.write(r90AbsorbData, 'Field4_r90Absorb.dat', format='fixed_width', overwrite=True)

#Table for combining r90 non-absroption data.
r90NoAbsorbData = (Table([r90NoAbsorb], names=['Radius 90']))
ascii.write(r90NoAbsorbData, 'Field4_r90NoAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining r50 absorption data.
r50AbsorbData = (Table([r50Absorb,], names=['Radius 50']))
ascii.write(r50AbsorbData, 'Field4_r50Absorb.dat', format='fixed_width', overwrite=True)

#Table for combining r50 non-absroption data.
r50NoAbsorbData = (Table([r50NoAbsorb], names=['Radius 50']))
ascii.write(r50NoAbsorbData, 'Field4_r50NoAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining Galaxy areas for absorbers
galAreaAbsorbData = (Table([galAreaAbsorb], names=['Galaxy Area']))
ascii.write(galAreaAbsorbData, 'Field4_galAreaAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining Galaxy areas for non-absorbers
galAreaNoAbsorbData = (Table([galAreaNoAbsorb], names=['Galaxy Area']))
ascii.write(galAreaNoAbsorbData, 'Field4_galAreaNoAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining sfrDensity data.
sfrAbsorbData = (Table([sfrDensAbsorb], names=['SFR Density Absorber']))
ascii.write(sfrAbsorbData, 'Field4_SFRDensAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining sfrDensity data.
sfrNoAbsorbData = (Table([sfrDensNoAbsorb], names=['SFR Density No Absorber']))
ascii.write(sfrNoAbsorbData, 'Field4_NoSFRDensAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining sfr data.
sfrAbsorbData = (Table([SFRAbsorb], names=['SFR Absorber']))
ascii.write(sfrAbsorbData, 'Field4_SFRAbsorb.dat', format='fixed_width', overwrite=True)

#Table for combining sfr data.
sfrNoAbsorbData = (Table([SFRNoAbsorb], names=['SFR Non Absorber']))
ascii.write(sfrNoAbsorbData, 'Field4_SFRNoAbsorb.dat', format='fixed_width', overwrite=True)
...
ascii.write(angDistData, 'Field4_AngularDistance.dat', format='fixed_width', overwrite=True)
...
ascii.write(angDistAbsorbData, 'Field4_AbsorbAngularDistance.dat', format='fixed_width', overwrite=True)
...
ascii.write(angDistNoAbsorbData, 'Field4_NoAbsorbAngularDistance.dat', format='fixed_width', overwrite=True)
...
ascii.write(data, 'HSTData4.dat', format='fixed_width', overwrite=True)

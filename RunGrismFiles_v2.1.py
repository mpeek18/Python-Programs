...
###############################################################################
###############################################################################    
def analyzeFits(finalName, fitsName, redshift, sciHeight):
    hdulist = fits.open(fitsName)
    ...
    if (redshift < 1.6):    #If redshift is less then z=1.6, check H-Alpha lines
        hAlphaGal = (redshift+1) * hAlphaWavelength
    ...
    elif (redshift > 1.6):  #If redshift is greater than z=1.6, check O-III lines
        OIIIGal = (redshift+1) * OIIIWavelength
        print ("O-III:", OIIIGal)
        print ()
...
def starFormation(lineFitDat, redshift):
...
#Try reading in ALL data from Filtered_Objects.dat file.
galaxyID = []
fluxHa = []
z = []
targetFile = ascii.read('Filtered_Objects.dat', data_start=1)
objID = targetFile['col2']
zShift = targetFile['col5']
HA = targetFile['col6']
...
for i in range(0, len(galaxyID)): #Loop through Galaxy ID #'s.    
    ...
    redshift = z[i]
    lineFitDat = fluxHa[i]
    #zFits = ascii.read('SDSS-J001453.19+091217.6-G141_00' + str(galID) + '.zfit.dat')
    #redshift = zFits['z_peak_spec']
    fitsName = 'SDSS-J001453.19+091217.6-G141_00' + str(galID) + '.2D.fits'
    finalName = 'FINAL-' + fitsName
    print ("Name:",fitsName)
    print (redshift)
    #lineFitDat = 'SDSS-J001453.19+091217.6-G141_00' + str(galID) + '.linefit.dat'
    
    #call functions
    sciHeight = processFits(fitsName)
    radius50, radius90 = analyzeFits(finalName, fitsName, redshift, sciHeight)
    sfr = starFormation(lineFitDat, redshift)
...
data = Table([ID, redShifts, R50, R90, starForm], names=['Galaxy ID', 'Redshifts', 'R50', 'R90', 'Star Formation'])

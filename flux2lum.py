...
# May 31, 2012
#
# Script for converting Ha line fluxes to luminosities & SFRs (Kennicutt)
#
# Call with:
# python flux2lum.py line_flux  line_z

def cosmocalc(zin):
    # Cosmology calculator ala Ned Wright (www.astro.ucla.edu/~wright)
    # input values = redshift
    # ouput values = age at z, distance in Mpc, kpc/arcsec, apparent to abs mag conversion

    if float(zin) > 100:
      z=float(zin)/299790.    # velocity to redshift
    else:
      z=float(zin)            # redshift
...
import os, sys
from math import log10, exp, sqrt, pi, sin, cos, acos
import pylab as pl
from pylab import *
...
if __name__ == '__main__':
    ...
    flux = float(sys.argv[1]) # ergs/s/cm^2
    zgal = float(sys.argv[2])


    (age,dist,kpcarc,mag, dl) = cosmocalc(zgal)
    
    mpc2cm = 3.085677581e+24
    lum = flux*4.*pi*(dl*mpc2cm)**2.
    sfr = (7.9*10**-42)*lum
    print ("Luminosity Distance (Mpc): %f" % (dl))
    print ("Line Luminosity (ergs): %f" % (lum))
    print ("SFR (M_sun / yr): %f" % (sfr))

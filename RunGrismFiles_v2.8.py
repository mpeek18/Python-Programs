...
newGalID = ['302', '359', '351', '303', '392', '236', '344', '284', '420', 
            '338', '427', '326', '269', '237', '306', '216', '262', '274', 
            '285', '305', '317', '274', '221', '226', '299', '364', '220', 
            '269', '419', '236', '420', '305', '317', '274', '221', '226']
...
galIDFile = raDecFile['col2'] #Galaxy ID's
angDist = raDecFile['col5'] #Angular distances
...
#Loop through total galaxy ID's and galaxy ID's from radec file, if both ID's
#match, write angular distance to new list.
totalAngDist = []
for i in range(0, len(totalID)):
    for j in range(0, len(radecGal)):
        if (totalID[i] == radecGal[j]):
            totalAngDist.append(angularDist[j])
print (totalAngDist)
...
#plt.clf()
fig, ax = plt.subplots()
ax.hist(sfrDensAbsorb, normed=True, histtype='step', label= 'MgII Detection (%i)' %len(sfrDensAbsorb))
ax.hist(sfrDensNoAbsorb, normed=True, histtype='step', label = 'MgII Non-Detection (%i)' %len(sfrDensNoAbsorb))
ax.set_ylim([0, 8])
plt.xlabel("SFR Surface Density $(M_{Sun} yr^{-1} Kpc^{-2})$")
plt.ylabel("Normalized Number")
plt.legend()
...
plt.savefig('AngularDist_vs_SFRDens')
plt.show()

plt.scatter(totalAngDist, totalSFR)
plt.xlabel('Angular Distance (arcsec)')
plt.ylabel('Star Formation Rate')
plt.savefig('AngularDist_vs_SFR')
plt.show()

...
# Iterate through cleaned fits and write data into new fits file
    ...
    fits.writeto('Final-goodsn-14-G141_' + str(galID) + '.2D.fits', newData, f[0].header, overwrite=True)
    f.close() 
    return sciHeight
...
#Begin placing aperture and annulus
    ...
    #Plot flux as radius increases over galaxy
    plt.clf()
    plt.plot(radArray, fluxArray)
    plt.show()
    
    fits.writeto('Crop-goodsn-14-G141_' + str(galID) + '.2D.fits', newImage, f[0].header, overwrite=True)
    f.close()
    
    print ()
    print ("Finished writng Fits cutout & plotted flux per radius!")
    print ()
###############################################################################
    """
    R90 Algorithm:
...
ascii.write(data, 'HSTData.txt', format='fixed_width', overwrite=True)

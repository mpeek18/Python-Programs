...
def analyzeFits(finalName, fitsName, zGal, sciHeight, hAlphaFlux):
    ...
    plt.savefig('CROP-J083852.05+025703.7-G141_00' + str(galID) + '.png', dpi=100)
    plt.show()
    ...
    plt.savefig('CROP-NB-SDSS-J083852.05+025703.7-G141_00' + str(galID) + '.png')
    plt.show()
    """
...
for i in range(0, len(galaxyID)): #Loop through Galaxy ID #'s.    
    ...
    if (zQual[i] == 'likely' or zQual[i] == 'good' or zQual[i] == 'probable' and galRedshift[i] < 1.6):
        ...
        #linefit.dat file
        try:
            lineFitDat = 'SDSS-J083852.05+025703.7' + str(frame) + 'G141_' + str(galID).zfill(5) + '.linefit.dat'    
            print (lineFitDat)
            datFile = ascii.read(lineFitDat)
            line = datFile['line']
            flux = datFile['flux']
